...
console.log('Loaded TOKEN:', process.env.TOKEN ? '[OK]' : '[MISSING]');
...
async function loadCommands() {
  ...
  const commands = [];
  const seenNames = new Set(); // Track command names
    ...
    for (const file of commandFiles) {
      ...
      if (command?.data && command?.execute) {
        ...
        if (seenNames.has(name)) {
          console.warn(chalk.yellow(`[Commands] Duplicate command name skipped: ${name} (${file})`));
  ...
  console.log('Registering commands:', commands.map(cmd => cmd.name));
